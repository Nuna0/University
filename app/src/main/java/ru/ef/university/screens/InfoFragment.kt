...
import ru.ef.university.databinding.FragmentInfoBinding
    ...
    fun addInfo(){
        ...
        viewModel.myFirstResponse.observe(this, Observer { response->
            with(binding){
                ...
                imgInst.setOnClickListener {
                    val i = Intent(Intent.ACTION_VIEW, Uri.parse(response.body()?.contactsInformation?.inst))
                    startActivity(i)
                }
